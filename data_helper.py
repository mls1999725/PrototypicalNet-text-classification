...
class PrototypicalData(object):
    ...
    def remove_stop_word(self, data: Dict[str, Dict[str, List[List[str]]]]) -> List[str]:
        ...
        for category, category_data in data.items():
            for sentiment, sentiment_data in category_data.items():
                all_words.extend(list(chain(*sentiment_data)))  # 多个容器合并成一个容器迭代
    ...
    def choice_support_query(self, task_data: Dict[str, List[List[int]]])\
        ...
        pos_support = random.sample(pos_samples, self.__num_support) # 所有的pos_support里随机采样num_suppor个
        ...
        labels = [label_to_index["1"]] * len(pos_query) + [label_to_index["-1"]] * len(neg_query)  # 只用query有label
